...
**Задачи:**
- [ ] Дополнить `LandingView.spec.ts`: hero-заголовок присутствует; вторая
...
- [ ] Hero-секция: заголовок, подзаголовок (1–2 предложения про OpsPilot),
      основная `<Button as-child>` -> `RouterLink to="/login"`.
- [ ] Features-секция: сетка (Tailwind grid, ≥4 карточки), каждая — иконка
...
- [ ] How-it-works-секция: 4 пронумерованных шага
      `тикет -> RabbitMQ -> агент (fake|claude) -> предложение -> человек`.
- [ ] Footer: `© 2026 OpsPilot` + ссылки-заглушки `<a href="#">`; классы
